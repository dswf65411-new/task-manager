...
## 防 context rot 設計（含「按需讀 detail」）
**第一趟（每輪）**：LLM 只看到「active 精簡索引（id/tag/title）」+ 最新對話。新增任務、改 tag/狀態、刪除都在這趟完成；detail 不進 context、done/backlog 歸檔後完全不餵。active 累積再多，進 LLM 的也只是每條約 15 token 的索引。

**第二趟（按需，lazy fetch）**：當這輪要「修改某既有 task 的具體內容/需求」時，第一趟會回報 `need_detail:[id]`；Python 才把那 1–2 條的完整 detail 餵回去，讓 LLM 在「現有內容基礎上」改寫（保留舊脈絡、不盲目覆蓋）。純狀態流轉的回合不會觸發第二趟。

→ 兼顧 rot-safe（平常只餵索引）與「能讀能改」（要編輯時才精準載入相關那幾條）。

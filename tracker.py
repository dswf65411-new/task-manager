...
- question：AI 有疑問、沒釐清、要問使用者、等使用者決定的**開發/設計議題**（例：用哪個 DB、要不要支援某功能、API 怎麼設計）。**注意**：純對話性的服務提議（「要不要我繼續做X」「需要我先 commit 嗎」「要我幫你裝嗎」）只是在問你要不要繼續，**不是** question task，不要建。
...
- **priority_hint**（**可選**欄位，不強求）：若對話中有提到「這個很急/優先/重要/緊急」或相反「不急/之後再說/不重要」，可以寫進去。值為 `high` / `normal` / `low`。沒有提到就**不要**亂填。
- **depends_on**（**可選**欄位，不強求）：若對話中有提到「這個要等 X 做完」「卡在 Y」「等 Z 完成才能做」，把依賴的 task id 寫進去，如 `["T-001"]`。沒有提到就不要寫。
...
1. **追求高 recall：寧可多建、絕不漏記。** 漏掉一個該追的任務代價極高；多建一個只要幾秒刪掉。所以判斷門檻往「建」的方向拉：
   ...
   - 例外（全 tag 適用）：**AI 自己對使用者的服務性提議**——「要不要我做X」「需不需要我X」「我可以幫你X」「要我幫你X 嗎」「接下來要不要我Y」——只是在問你要不要繼續，**本身不建任何 task（todo/question/其他都不建）**，等使用者明確同意後才算。
...
# 輸出格式（第一趟）
{"ops": [
  {"action": "add", "tag": "todo", "title": "...", "detail": "...", "priority_hint": "high", "depends_on": ["T-001"]},
    ...
    for op in ops:
        ...
        try:
            if action == "add":
                ...
                if op.get("priority_hint") in ("high", "normal", "low"):
                    item["priority_hint"] = op["priority_hint"]
                if op.get("depends_on"):
                    item["depends_on"] = op["depends_on"]
                ...
                if "priority_hint" in op:
                    item["priority_hint"] = op["priority_hint"] if op["priority_hint"] in ("high", "normal", "low") else None
                if "depends_on" in op:
                    item["depends_on"] = op["depends_on"]
    ...
    for t in ALL_TAGS:
        ...
        for it in rows:
            ...
            if it.get("priority_hint"):
                suffix += f" [{it['priority_hint']}]"
            if it.get("depends_on"):
                suffix += f" 🔗{','.join(it['depends_on'])}"
            lines.append(f"- **{it['id']}** {it.get('title','')}{suffix}  ·_{it.get('updated','')}_")
            if it.get("detail"):

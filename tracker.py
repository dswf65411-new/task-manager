...
def main():
    ...
    try:
        ...
        t0 = time.time()
        # 第一趟：只看精簡索引（rot-safe）→ 拿 ops + need_detail
        text, usage = call_deepseek(build_pass1_user(active_index, exchange))
        ...
        if need:
            ...
            if details:
                text2, usage2 = call_deepseek(build_pass2_user(details, exchange))
        ...
        log(taskdir, f"done {dt:.1f}s ops={len(ops)} applied={len(applied)} "
                     f"need_detail={len(need)} pass2={'Y' if usage2 else 'N'} "
